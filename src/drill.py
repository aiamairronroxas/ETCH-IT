...
def plot_drill_layer(ax, file_path):
    ...
    for primitive in layer.primitives:
        if isinstance(primitive, Circle) or hasattr(primitive, 'position'):
            ...
            d = 0.5  # Default drill size visualization
...
def plot_excellon(file_path):
    ...
    ax.autoscale_view()
    ax.set_aspect('equal')
    ax.set_title(f"Drill Verification: {os.path.basename(file_path)}")
    ...
    try:
        ...
        pass # Fallback for non-tk backends
    
    plt.show() # This is a blocking call; user must close plot to return to GUI
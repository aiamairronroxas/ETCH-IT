...
class EtchItApp:
    def __init__(self, root):
        ...
        self.f4_cfg = FRAME_CONFIGS["f4_btn"]
        ...
        self.main_content.place(
            relx=self.cfg["sidebar_rel_width"],
            rely=0,
            relwidth=1 - self.cfg["sidebar_rel_width"],
            relheight=1
        )
        ...
        self.subtitle_label = tk.Label(
            self.main_content,
            text="PCB Etching and Drilling Device for Chemical-Free Fabrication",
            bg=self.colors["main_content_bg"],
        ...
        # Compute equal layout for the 3 bottom buttons
        ...
        self.mid_canvas.place(
            x=self.center_frame_x,
            y=self.center_frame_y,
            width=self.center_frame_w,
            height=self.center_frame_h
        )
        ...
        self.mid_canvas4.place(x=f4["pos_x"], y=f4["pos_y"])
        self.home_elements.append(self.mid_canvas4)
        self.draw_f4_button()
        ...
        if hasattr(self, 'right_standalone_img'):
            self.standalone_icon_label = tk.Label(
                self.main_content,
                image=self.right_standalone_img,
                bg=self.colors["main_content_bg"]
            )
            self.standalone_icon_label.place(
                x=self.right_standalone_cfg["pos_x"],
    ...
    def configure_bottom_buttons(self):
        ...
        total_width = self.center_frame_w
        btn_width = int((total_width - (2 * gap)) / 3)

        configs = [self.f2_cfg, self.f3_cfg, self.f4_cfg]

        for i, cfg in enumerate(configs):
            cfg["width"] = btn_width
            cfg["height"] = btn_height
            cfg["pos_x"] = self.center_frame_x + i * (btn_width + gap)
            cfg["pos_y"] = btn_y
    ...
    def draw_f1_content(self):
        ...
        self.mid_canvas.create_text(
            72,
            150,
            text="Direct conversion to NC file",
            fill="#FFFFFF",
            font=("Arial", 14),
            anchor="w"
        )
        ...
        def draw_gen_btn(color):
            ...
            self.start_btn_canvas.create_text(
                ...
                25,
                text="GENERATE!",
                fill=self.colors["btn_text_white"],
                font=("Arial", 10, "bold")
            )
    ...
    def draw_f2_button(self, hover=False):
        ...
        if hasattr(self, 'f2_icon_img'):
            self.mid_canvas2.create_image(
                start_x + (f["icon_size"][0] / 2),
                f["height"] / 2,
                image=self.f2_icon_img
            )

        self.mid_canvas2.create_text(
            start_x + f["icon_size"][0] + f["spacing"],
            f["height"] / 2,
            text=f["text"],
            fill=f["text_color"],
            font=f["font"],
    ...
    def draw_f3_button(self, hover=False):
        ...
        if hasattr(self, 'f3_icon_img'):
            self.mid_canvas3.create_image(
                start_x + (f["icon_size"][0] / 2),
                f["height"] / 2,
                image=self.f3_icon_img
            )

        self.mid_canvas3.create_text(
            start_x + f["icon_size"][0] + f["spacing"],
            f["height"] / 2,
            text=f["text"],
            fill=f["text_color"],
    ...
    def draw_f4_button(self, hover=False):
        self.mid_canvas4.delete("all")
        f = self.f4_cfg
        c1 = self.colors["f4_hover_start"] if hover else self.colors["f4_grad_start"]
        c2 = self.colors["f4_hover_end"] if hover else self.colors["f4_grad_end"]
        ...
        if hasattr(self, 'f4_icon_img'):
            self.mid_canvas4.create_image(
                start_x + (f["icon_size"][0] / 2),
                f["height"] / 2,
                image=self.f4_icon_img
            )

        self.mid_canvas4.create_text(
            start_x + f["icon_size"][0] + f["spacing"],
            f["height"] / 2,
            text=f["text"],
            fill=f["text_color"],
        ...
        self.mid_canvas4.bind("<Button-1>", self.on_drill_click)
        self.mid_canvas4.bind("<Enter>", lambda e: self.draw_f4_button(hover=True))
        self.mid_canvas4.bind("<Leave>", lambda e: self.draw_f4_button(hover=False))
    ...
    def create_nav_item(self, text, icon, pack_side="top"):
        ...
        canvas.pack(
            side=pack_side,
            padx=self.cfg["button_padding_x"],
            pady=(10, 20 if text == "Exit" else 0)
        )
    ...
    def draw_btn(self, canvas, color, text, icon):
        ...
        if icon:
            canvas.create_image(w / 2, h / 2 - 10, image=icon)

        canvas.create_text(
            w / 2,
            h / 2 + (18 if icon else 0),
            text=text,
            fill=self.colors["sidebar_text"],
            font=("Arial", self.cfg["nav_font_size"], "bold")
        )
    ...
    def typewriter_effect(self, char_index):
        ...
        if char_index <= len(phrase):
            ...
            self.root.after(500, lambda: self.jump_animation(0, 2))
    ...
    def jump_animation(self, jump_count, total_jumps):
        if jump_count < total_jumps:
            ...
            self.root.after(1500, self.next_phrase)
    ...
    def sync_sidebar_buttons(self, event=None):
        ...
        if new_size > 20:
            for btn in [self.home_btn, self.stop_btn, self.exit_btn, self.hide_btn]:
                ...
                self.root.after(
                    10,
                    lambda b=btn: self.draw_btn(
                        b["canvas"],
                        self.colors["sidebar_bg"],
                        b["text"],
                        b["icon"]
                    )
                )
